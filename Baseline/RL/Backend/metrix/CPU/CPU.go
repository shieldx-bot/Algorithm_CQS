...
func StartCPUP95Logger() {
	...
	logger, cleanup, err := newCPUMetricsFileLogger()
	if err != nil {
		// If we can't create a file logger, disable CPU metrics without printing.
		return
	}
	defer func() {
		_ = cleanup()
	...
	if err := syscall.Getrusage(syscall.RUSAGE_SELF, &prevRU); err != nil {
		...
		writeJSONLine(logger, map[string]interface{}{
			"time":  timeStr,
			"ts":    timeStr,
